...
from modular.data_setup import (
    TripletDataset,
    get_train_test_val_dataset,
    triplet_collate_fn,
)
from modular.engine import (
    calculate_triplet_accuracy,
    clear_cuda_cache,
    process_triplet_batch,
    validate_model,
)
...
train_loader = DataLoader(
    train_dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=triplet_collate_fn
)
val_loader = DataLoader(
    val_dataset, batch_size=BATCH_SIZE, shuffle=False, collate_fn=triplet_collate_fn
)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, batch in enumerate(progress_bar):
        ...
        with torch.no_grad():
            accuracy = calculate_triplet_accuracy(
        ...
        if global_step % logging_steps == 0:
            ...
            logger.info(
                f"Step {global_step} - Loss: {avg_loss:.4f}, Accuracy: {avg_acc:.4f}"
            )
        ...
        if global_step % eval_steps == 0 and (batch_idx + 1) < len(train_loader):
            ...
            val_loss, val_accuracy = validate_model(
                model, val_loader, triplet_loss, accelerator
            )
            logger.info(
            ...
            if val_accuracy > best_val_accuracy:
                ...
                if accelerator.is_main_process:
                    accelerator.save_state("models/best_model_checkpoint")
                    logger.info(
                        f"New best model saved with accuracy: {val_accuracy:.4f}"
                    )
